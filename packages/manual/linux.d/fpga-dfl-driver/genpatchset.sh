...
#    git remote add linux-stable git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git
    ...
    mv dfl_patches ${workingDir}/dfl_patches_branch
    git checkout ${DFL_TAG} 
    git format-patch -o dfl_patches --first-parent --no-merges v${KERNEL_VERSION}..${DFL_TAG}
    mv dfl_patches ${workingDir}/dfl_patches_tag
popd ||exit 1
